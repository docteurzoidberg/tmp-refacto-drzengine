...
void Widget::DrawPixel(int px, int py, Color p) {
  ...
  if (x >= gfx->GetScreenWidth())
    return;
  if (y >= gfx->GetScreenHeight())
    return;
  gfx->DrawPixel(x, y, p);
}
...
void Widget::DrawText(std::string text, int px, int py, Color c) {
  ...
  int x = px + screenX;
  int y = py + screenY;
  // Check if the text is out of the widget bounds
...
void Widget::DrawLine(int x1, int y1, int x2, int y2, Color c) {
  ...
  int x_ = x2 + screenX;
  int y_ = y2 + screenY;
  // Check if the line is out of the widget bounds
...
void Widget::FillRect(int x, int y, int w, int h, Color c) {
  ...
  int x_ = x + screenX;
  int y_ = y + screenY;
  // TODO: Check if the rectangle is out of the widget bounds
