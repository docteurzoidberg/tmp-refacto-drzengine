...
static const Color GREY(192, 192, 192), DARK_GREY(128, 128, 128),
...
inline Color::Color(uint32_t p) { n = p; }

inline bool Color::operator==(const Color &p) const { return n == p.n; }
...
inline Color Color::operator*(const Color &p) const {
  uint8_t nR =
      uint8_t(std::min(255.0f, std::max(0.0f, float(r) * float(p.r) / 255.0f)));
  uint8_t nG =
      uint8_t(std::min(255.0f, std::max(0.0f, float(g) * float(p.g) / 255.0f)));
  uint8_t nB =
      uint8_t(std::min(255.0f, std::max(0.0f, float(b) * float(p.b) / 255.0f)));
  uint8_t nA =
      uint8_t(std::min(255.0f, std::max(0.0f, float(a) * float(p.a) / 255.0f)));
...
inline Color ColorF(float red, float green, float blue, float alpha) {
  return Color(uint8_t(red * 255.0f), uint8_t(green * 255.0f),
...
public:
  ...
  void SetSampleMode(Mode mode) { modeSample = mode; }
  ...
  Color GetPixel(int32_t x, int32_t y) const {
    if (modeSample == Sprite::Mode::NORMAL) {
      ...
      if (modeSample == Sprite::Mode::PERIODIC)
        ...
        return colData[std::max(0, std::min(y, height - 1)) * width +
                       std::max(0, std::min(x, width - 1))];
    }
  }
  ...
  Color SampleBL(float u, float v) const {
    ...
    Color p4 = GetPixel(std::min(x + 1, (int)width - 1),
                        std::min(y + 1, (int)height - 1));

    return Color((uint8_t)((p1.r * u_opposite + p2.r * u_ratio) * v_opposite +
  ...
  Color *GetData() { return colData.data(); }
};
...
public:
  ...
  static float Length(vec3d &v) { return sqrtf(DotProduct(v, v)); }
  ...
  static vec3d IntersectPlane(vec3d &plane_p, vec3d &plane_n, vec3d &lineStart,
                              vec3d &lineEnd) {
...
public:
  ...
  static Matrix4x4 CreateRotationMatrix(float angleX, float angleY,
                                        float angleZ) {
...
class Model {
public:
  Model(std::vector<vec3d> *verts, std::vector<face> *faces)
  ...
  void SetupModel() {
    ...
    _loadTriRefs();
    // std::cout << "Model loaded with " << tris.size() << " triangles" <<
    // std::endl;
  }
...
public:
  ...
  virtual void DrawPartialSprite(int x, int y, Sprite *sprite, int ox, int oy,
                                 int w, int h) = 0;
  virtual void DrawPartialSprite(vi2d pos, Sprite *sprite, vi2d srcPos,
                                 vi2d size) = 0;
...
private:
  ...
  inline static void _charBounds(unsigned char c, int16_t *x, int16_t *y,
                                 int16_t *minx, int16_t *miny, int16_t *maxx,
                                 int16_t *maxy);
  inline static void _getTextBounds(const char *str, int16_t x, int16_t y,
                                    int16_t *x1, int16_t *y1, uint16_t *w,
                                    uint16_t *h);
  inline static void _getTextBounds(const std::string &str, int16_t x,
